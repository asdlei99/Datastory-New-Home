	...
	<div id="site-footer">
		<div id="consult-sec" class="section">
			<div class="consult-wrap">
				...
				<a @click="emitService()" href="javascript:;" id="consult-button">
					<span><img src="../assets/imgs/index/consultIcon.png" alt="我要咨询"></span>
					我要咨询
				</a>
			</div>
		</div>
		<div id="link-sec" class="section">
			<div class="link-col">
				<div class="ul-div">
					...
					<ul>
						<li><a @click.prevent="pageGoTo('about', 'companyInfo')">数说简介</a></li>
						<li><a @click.prevent="pageGoTo('about', 'teamInfo')">团队介绍</a></li>
						<li><a @click.prevent="pageGoTo('about', 'newsInfo')">数说新闻</a></li>
					</ul>
				</div>
				<div class="ul-div">
					<div class="link-header">解决方案</div>
					<ul>
						<li><a @click.prevent="pageGoTo('pands/solution', 'daiyanAnchor')">代言人甄选与监测</a></li>
						<li><a @click.prevent="pageGoTo('pands/solution', 'zongyiAnchor')">综艺节目监测</a></li>
						<li><a @click.prevent="pageGoTo('pands/solution', 'pinpaiAnchor')">品牌资产与健康度</a></li>
						<li><a @click.prevent="pageGoTo('pands/solution', 'xiaofeizheAnchor')">消费者洞察</a></li>
					</ul>
				</div>
				<div class="ul-div">
					<div class="link-header">数说产品</div>
					<ul>
						<li><a @click.prevent="pageGoTo('pands/product', 'radarAnchor')">数说雷达</a></li>
			...
			<div class="link-col link-col_contact-col">
				<div class="contact-div">
					<div class="link-header">
						<span class="contact-icon"><img src="../assets/imgs/index/telIcon.png" alt=""></span>
						联系电话
					</div>
				...
				<div class="contact-div">
					<div class="link-header">
						<span class="contact-icon"><img src="../assets/imgs/index/addrIcon.png" alt=""></span>
						联系地址
					</div>
					<div class="contact-bd">
						<ul>
							<li v-for="item in contactAddr" v-text='item.city' v-bind:class='{selectedAddr: $index == current}' v-on:click="toggleAddr($index)"></li>
						</ul>
						<div id="contact-addr-detail">
							<p v-for="item in contactAddr" v-html="item.addr" v-if="$index == current"></p>
						</div>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			emitService: function() {
				this.$dispatch('emitService', 'service')
			},
			pageGoTo: function(page, anchor) {
				var hash = window.location.hash;
				// console.log(hash, page, hash.indexOf(page));
				if (hash.indexOf(page) < 0) {
					window.location.hash = '#!/' + page + '?anchor=' + anchor;
				} else {
					var target = $('#' + anchor);
					if (target) {
		...
		events: {
			pageGoHook: function(obj) {
				this.pageGoTo(obj.page, obj.anchor);
			}
		}
	}
</script>
